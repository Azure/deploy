...
function configureMocks(inputYaml: string) {
  ...
  mockCore.getInput.mockImplementation((inputName) => {
    ...
    if (typeof value !== "string") {
      throw new Error(`Only string values are supported (parsing ${inputName})`);
    }
  ...
  mockCore.error.mockImplementation(message => { result.errors.push(removeColors(message)); console.error(message); });
...
export async function runAction(getYaml: (data: EnvironmentData) => string, expectSuccess: boolean = true) {
